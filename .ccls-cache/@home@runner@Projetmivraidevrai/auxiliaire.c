      ...
      if (colonnep[ordonneepion][abcissepion] > 2) { // cas d'une pièce promue
        ...
        case 2: {
          ...
          if (num_j == 1) {
            ...
            int pion_2 = colonnep[ordonneepion - 1][abcissepion];
            prom[ordonneepion - 1][abcissepion] = 0;
            prom[ordonneedeplace - 1][abcissepion] = pion_prom_2;
            colonnep[ordonneedeplace - 1][abcissedeplace] = pion;
...
void promouvoir(int **colonnep, int **prom, int dernier_coup) {

  int ordo = dernier_coup / 10;
  int abci = dernier_coup % 10;

  int indice = prom[ordo][abci];
  initialisation(colonnep, 2);
  init_prom(prom);
  if (indice < 9) {
...
int deplacement_possible_prom(int abcissepion, int ordonneepion,
  ...
  if (num_joueur == 1) { // joueur des x
    ...
    if (((ordonneedeplace - ordonneepion) == 1) &&
        (colonnep[abcissepion][ordonneedeplace] != 0)) { // pousser une pièce
      if (colonnep[abcissepion][ordonneedeplace] == 3) {
        printf("\nIl est impossible de pousser une pièce promue !\n");
        return 0;
      }
      if (colonnep[abcissepion][ordonneedeplace] == 2) {
      ...
      if (colonnep[abcissepion][ordonneedeplace + 1] != 0) {
    ...
    if (ordonneepion < ordonneedeplace) {
      printf("\nIl est impossible de reculer !\n");
      return 0;
      printf("\n%d\n",colonnep[abcissepion][ordonneedeplace]);
      if (((ordonneepion - ordonneedeplace) == 1) &&
          (colonnep[abcissepion][ordonneedeplace] != 0)) { // pousser une pièce
        printf("\npousser1\n");
        if (colonnep[abcissepion][ordonneedeplace] == 4) {
          printf("\n2Il est impossible de pousser une pièce promue !\n");
          return 0;
        }
        if (colonnep[abcissepion][ordonneedeplace] == 1) {
